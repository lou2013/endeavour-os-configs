...
function get_brightness {
    brightness_percentage=$(brightnessctl | grep -Po '[0-9]+(?=%)' | head -n 1)

    if [ "$brightness_percentage" -gt 100 ]; then
        brightness_percentage=100
    fi
    if [ "$brightness_percentage" -lt 5 ]; then
...
case $1 in
    ...
    get_brightness
    brightness_percentage=$(( "$brightness_percentage" + "$brightness_step" ))
    if [ "$brightness_percentage" -gt 100 ]; then
        brightness_percentage=100
    fi
    if [ "$brightness_percentage" -lt 5 ]; then
    ...
    brightnessctl set $brightness_percentage%
